...
import BarCharts from "../barChartData/BarChartData";
...
function Home() {
  ...
  function handleIdgenerator() {
    ...
    if (name) {
      ...
      const randomAlphabet = alphabet.charAt(
    ...
    <div className="border w-[80%] m-auto flex flex-col rounded-xl gap-3 p-10 bg-[cadetblue]">
      <div className="flex rounded-xl">
        <div className="flex m-auto gap-10 w-[81%] justify-center sm:flex-wrap">
          <div className="border p-3 w-[46%] rounded-xl flex flex-col gap-3 text-center">
            ...
            <div className="flex gap-3 justify-center flex-wrap ">
              <button
                ...
                className="bg-green-400 rounded pl-4 pr-4 text-white pb-1 font-bold"
              >
                +
              </button>
              <button
                onClick={resetHanlder}
                className="bg-yellow-400 pl-4 rounded pr-4 text-white pb-1 font-bold"
              >
                Reset
              </button>
              <button
                onClick={decreaseHanlder}
                className="bg-red-800 pl-4 rounded pr-4 text-white pb-1 font-bold"
              >
                -
              </button>
            </div>
          ...
          <div className="border p-3 w-[50%] rounded-xl text-white">
            ...
            {flag1 && !showList && userData.address && (
              <p className={property}>Address: {userData.address}</p>
            )}
            {flag1 && !showList && userData.email && (
              <p className={property}>Email: {userData.email}</p>
            )}
            {flag1 && !showList && userData.phone && (
              <p className={property}>Phone: {userData.phone}</p>
            )}

            {showList && (
              <ul style={{ listStyleType: "square" }} className="p-3">
                {flag1 && userData.address && (
            ...
            {flag1 && (
              <div className="flex gap-2 mt-3 flex-wrap">
                <button
                  ...
                  className="bg-blue-400 pl-4 pr-4 text-white pb-1 rounded-lg"
                >
                  Bold
                </button>
                <button
      ...
      <div className="flex justify-center gap-10 md:flex-col sxm:border-0">
        <div className="border rounded-xl w-[38%] h-[40%] flex flex-col items-center p-4 gap-3 text-white md:w-auto">
          ...
          />
          {!name && flag && (
            <p className="text-red-700">Please Enter Name !!</p>
          )}

          <p className="w-full border p-3 rounded font-bold">
            Id generator : {idGenerator}
          </p>
          <button
            onClick={handleIdgenerator}
            className="w-full p-2 bg-violet-700	rounded-lg text-white font-bold"
          >
            Save
          </button>
        ...
        <div className="border p-3 w-[40%] rounded-xl md:w-auto">
          <div className="flex flex-col  items-start">
            ...
            <input
              ...
              className="border rounded w-full pl-2 p-3 focus:outline-none"
              type="text"
              placeholder="Enter your address"
            />
            {flag2 && !userData.address && (
          ...
          <div className="flex flex-col  items-start">
            <label className="text-lg text-white font-bold" for="email">
              Enter your email: *
            </label>
            <input
              onChange={(e) =>
            ...
            />
            {flag2 && !userData.email && (
              <p className="text-red-500">Please Enter your email</p>
            )}
          </div>

          <div className="flex flex-col  items-start">
            <label className="text-lg text-white font-bold" for="number">
              Enter your number: *
            </label>
            <input
              onChange={(e) =>
            ...
            />
            {flag2 && !userData.phone && (
              <p className="text-red-500">Please Enter your number</p>
            )}
          </div>
          ...
          <div className="flex flex-col mt-5 items-start hover:bg-violet-900">
            <button
              ...
              className="bg-violet-700	 w-full p-2 rounded-lg text-white font-bold"
              type="submit"
            >
              Submit
            </button>
      ...
      <div className="w-[81%] m-auto">
        <div className="border rounded">
          <div
            style={{ width: `${count}%`, padding: "10px" }}
